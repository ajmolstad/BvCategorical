...
outPred.dev <- BvCat.predict(Xtest = Xtest, modfit, lambda = modfit$lambda.vec[devInds[1,1]], gamma = modfit$gamma.vec[devInds[1,2]]) # prediction using tuning parameters which minimized deviance
...
Finally, we can extract the regression coefficients using $\texttt{BvCat.coef}$. It is important to use this function rather than tryign to extracty the coefficients from the output of $\texttt{BvCat.cv}$ since $\texttt{BvCat.coef}$ will return coefficients on the original scale. 

When we use the argument $\texttt{type="matrix"}$, we get the matricized version of $\hat{\boldsymbol{\beta}}$, $\hat\beta$, which is given by
...
Using the argument $\texttt{type="array"}$ instead returns $\hat{\boldsymbol{\beta}}$, the array-valued estimate. 
```{r getCoefArray, include = TRUE, cache=TRUE}
outBetaArray <- BvCat.coef(modfit, type="array")
str(outBetaArray)
```

Let's not verify that the results of both functions agree in terms of their predicted probabilties. 
```{r checkProbs, include=TRUE, cach=TRUE}
outPred.check <- BvCat.predict(Xtest = Xtest[1,,drop=FALSE], modfit, type="probabilities") 
outCoef.check <- BvCat.coef(modfit)
l0 <- exp(outCoef.check$b0 + Xtest[1,,drop=FALSE]%*%outCoef.check$beta)
rbind(c(l0/sum(l0)),c(unlist(outPred.check))) ## both approaches agree
