...
---
In this document, we provide a short tutorial on how to use the $\texttt{BvCategorical}$ R package. **If you encounter any errors or strange behavior, please report the issue at https://github.com/ajmolstad/BvCategorical**. We start by downloading this package from GitHub.
...
library(Matrix)
library(MASS)
source("~/Documents/GitHub/BvMultinom/R/BvCategorical_Functions.R")
```
...
To fit the model, use the function $\texttt{BvCat.cv}$.The arguments are as follows:
  ...
  - $\texttt{X}$: an $n \times p$ matrix of predictors, unstandardized. 
  - $\texttt{Y}$: a list with two components, each corresponding to one of the two response variables. Elements of each component should be integers from $1$ to $G$ where $G$ is the number of response categories for that response. 
  ...
  - $\texttt{standardize}$: $\texttt{TRUE/FALSE}$ should the predictors be standardized for model fitting?
   ...
   - $\texttt{quiet}$:  $\texttt{TRUE/FALSE}$ Print some progress?
   - $\texttt{inner.quiet}$: $\texttt{TRUE/FALSE}$ Print updates from proximal gradient descent algorithm? Should be used only for diagnosing tolerance since output can be verbose. 
...
modfit <- BvCat.cv(X, Y, ngamma = 10, 
                  lambda.vec = rev(10^seq(-4, -1, length=5)), 
...
$$ \hat\beta_{l,f(j,k)} = \hat{\boldsymbol{\beta}}_{l,j,k}, \quad\quad (l,j,k) \in \left\{1, \dots, p\right\} \times \left\{1, \dots, J \right\} \times \left\{1, \dots, K\right\}.$$
...
Using the argument $\texttt{type="array"}$ instead returns $\hat{\boldsymbol{\beta}}$, the tensor-valued estimate. 
